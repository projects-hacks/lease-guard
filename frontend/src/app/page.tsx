...
export default function Home() {
  return (
    <div className="flex flex-col min-h-[calc(100dvh-3.5rem)] relative overflow-hidden bg-background">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-primary/20 blur-[120px] rounded-full point-events-none animate-float" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-500/10 blur-[120px] rounded-full point-events-none animate-float" style={{ animationDelay: "3s" }} />

      {/* Hero */}
      <section className="relative z-10 flex-1 flex flex-col items-center justify-center space-y-8 py-20 px-4 text-center md:py-32">
      ...
      {/* Features */}
      <section className="relative z-10 py-16 md:py-24 px-4 bg-black/20 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            <h2 className="text-3xl font-bold tracking-tight mb-4 text-gradient">Comprehensive Protection</h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Everything you need to secure your home and protect your wallet, powered by advanced AI models.</p>
          </div>
            ...
            {FEATURES.map((f, idx) => {
              ...
              return (
                <Link
                  key={f.href}
                  href={f.href}
                  className={`group flex flex - col items - start space - y - 4 p - 6 rounded - 2xl glass - card transition - all duration - 300 hover: -translate - y - 1 hover: shadow - 2xl ${f.borderHover} animate - fade -in -up`}
                  style={{ animationDelay: `${0.3 + (idx * 0.1)} s` }}
                >
                  <div className={`p - 4 rounded - xl ${f.bg} ${f.bgHover} transition - colors ring - 1 ring - white / 5`}>
      ...
      {/* Footer */}
      <footer className="relative z-10 py-8 px-4 border-t border-white/5 bg-black/40">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
          <p className="text-sm text-muted-foreground font-medium">
            © 2026 LeaseGuard • DeveloperWeek Hackathon
          </p>
          <div className="flex items-center flex-wrap justify-center gap-3 text-xs text-muted-foreground/60 font-medium">
