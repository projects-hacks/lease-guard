...
import { Loader2, DollarSign, MapPin, Bed, CheckCircle, AlertTriangle, XCircle } from "lucide-react";
    ...
    const handleAnalyze = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/v1/rent/analyze", {
                ...
                body: JSON.stringify({
                    zipCode: zip,
                    state: "CA", // Default for now or extract from zip API
        ...
        <div className="w-full max-w-lg mx-auto bg-card border rounded-xl shadow-sm p-6">
            ...
            {result && (
                <div className="mt-8 border-t pt-6 animation-fade-in">
                    <div className={cn(
                        "p-4 rounded-lg flex items-start gap-4 mb-4",
                        ...
                        <div>
                            ...
                            <p className="text-sm mt-1">
                                Avg (est): <b>${result.market_stats.average}</b> <br />
                                Range: ${result.market_stats.min} - ${result.market_stats.max}
                            </p>
                        </div>
                    </div>
